...

{
    _displayName = M_CONFIG(getText,"VirtualItems",_x,"displayName");
     _className = _x;     _price = -1;     if (isClass(missionConfigFile >> "CfgDynamicMarketPrice" >> _className)) then {         { if (_x # 0 == _className) exitWith { _price = _x # 1; }; } forEach BoursePublicPrice;     } else {         _price = M_CONFIG(getNumber,"VirtualItems",_className,"buyPrice");     };
