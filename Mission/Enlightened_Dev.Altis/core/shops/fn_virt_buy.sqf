...
_price = -1; if (getNumber(missionConfigFile >> "CfgDynamicMarketPrice" >> _type >> "synchronizeBuyPrice") == 1) then {         { if (_x # 0 == _type) exitWith { _price = _x # 1; }; } forEach BoursePublicPrice; } else {         _price = lbValue[2401,(lbCurSel 2401)]; }; 
