...
_price = -1; if (isClass(missionConfigFile >> "CfgDynamicMarketPrice" >> _type)) then { { if (_x # 0 == _type) exitWith { _price = _x # 1; }; } forEach BoursePublicPrice; } else { _price = M_CONFIG(getNumber,"VirtualItems",_type,"sellPrice"); }; 
...
if ([false,_type,_amount] call life_fnc_handleInv) then {
	...
	if (isClass(missionConfigFile >> "CfgDynamicMarketPrice" >> _type)) then { _amoutOfType = player getVariable _type; if (!isNil "_amoutOfType") then { _amoutOfType = _amoutOfType + _amount; player setVariable [_type, _amoutOfType]; } else { player setVariable [_type, _amount]; };}; 
};
